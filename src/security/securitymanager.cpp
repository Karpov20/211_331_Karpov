...
static constexpr unsigned char kIntegrityBlock[] = "ShipmentLedgerIntegrityBlock_v1";
...
public:
        ...
        : QObject(parent)
    {
        if (!verifyStaticBlock()) {
            requestShutdown(tr("Обнаружено изменение контрольной суммы."));
            return;
        }
        ...
        if (m_textSectionChecksum == 0) {
            requestShutdown(tr("Не удалось инициализировать контрольную сумму приложения."));
            return;
        }
...
private:
    ...
    void scheduleIntegrityTimer()
    {
        ...
        connect(timer, &QTimer::timeout, this, [this]() {
            ...
            if (current == 0 || current != m_textSectionChecksum) {
                requestShutdown(tr("Целостность кода нарушена."));
            }
        });
        timer->start();
#endif
        ...
        std::thread([this]() {
            while (!m_shutdownRequested.load(std::memory_order_acquire)) {
                if (isDebuggerPresentExtended()) {
                    requestShutdown(tr("Обнаружена активная отладка процесса."));
                    break;
                }
    ...
    void requestShutdown(const QString &reason) const
    {
        ...
        QMetaObject::invokeMethod(qApp, [reason]() {
            QMessageBox::critical(nullptr, QObject::tr("Защита приложения"), reason);
